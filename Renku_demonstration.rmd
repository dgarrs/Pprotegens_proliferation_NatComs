...
After DADA2 pipeline is done, proceed with the data analysis.
...
for (i in unique(Class_abundtax_top$Class)){
  ...
  #check effects of ComState, Environmet and Inoculant per class
  modelClass  <- lm(class_filt$abund ~ class_filt$ComState * class_filt$Environment * class_filt$Inoculant, data = class_filt)
...
## 7.6 Differences in Gammaproteobacteria removing CHA0 counts
...
noCHA0_Class_abund.melt = noCHA0_Class_abund %>% select(-Replicate, -RGBcol) %>% melt(value.name = "abund")
...
#Store results
noCHA0_anovaClassres = data.frame()
