...
#BiocManager::install(c("phyloseq", "pals", "ggpubr", "ggh4x", "agricolae"), ask = FALSE)
...
```{r}
#getting phyloseq_transform_css function from metagMisc
...
diver.ob = plot_richness(ps2, x="Names", measures=c("Observed"))+
  geom_bar(position = "identity", stat = "summary", aes(y=value-100), fill="darkgrey", fun=mean)+
  geom_errorbar(stat = "summary", width=0.75, aes(y=value-100), fun=mean) + 
...
diver.sha = plot_richness(ps2, x="Names", measures=c("Shannon"))+
  geom_bar(position = "identity", stat = "summary", aes(y=value-2), fill="darkgrey", fun=mean)+
  geom_errorbar(stat = "summary", width=0.75, aes(y=value-2), fun=mean) + 
...
ggarrange(observed_all, shannon_all, nrow = 2)
```
